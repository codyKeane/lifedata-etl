...
def _fill_baseline(db, source_module, base_value=50.0, days=14):
    """Insert N days of baseline data ending yesterday."""
    for i in range(days):
        date_str = _days_ago(days - i)
...
class TestAnomalyDetectorZScore:
    def test_no_anomalies_normal_value(self, db):
        ...
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _insert_metric_day(db, "device.battery", _today(), 80.0)

        detector = AnomalyDetector(db, zscore_threshold=2.0)
        anomalies = detector.check_today(_today())
    ...
    def test_extreme_high_flagged(self, db):
        ...
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _insert_metric_day(db, "device.battery", _today(), 200.0)
    ...
    def test_extreme_low_flagged(self, db):
        ...
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _insert_metric_day(db, "device.battery", _today(), 0.0)
    ...
    def test_insufficient_baseline_skipped(self, db):
        ...
        _insert_metric_day(db, "device.battery", _days_ago(10), 80.0)
        _insert_metric_day(db, "device.battery", _days_ago(9), 80.0)
        _insert_metric_day(db, "device.battery", _today(), 200.0)
    ...
    def test_anomalies_sorted_by_severity(self, db):
        ...
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _fill_baseline(db, "mind.mood", base_value=7.0, days=14)
        # Both extreme, but battery more so
        _insert_metric_day(db, "device.battery", _today(), 300.0)
        _insert_metric_day(db, "mind.mood", _today(), 20.0)
    ...
    def test_severity_labels(self, db):
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _insert_metric_day(db, "device.battery", _today(), 500.0)
    ...
    def test_custom_threshold(self, db):
        ...
        _fill_baseline(db, "device.battery", base_value=80.0, days=14)
        _insert_metric_day(db, "device.battery", _today(), 90.0)

        low_threshold = AnomalyDetector(db, zscore_threshold=0.5)
        high_threshold = AnomalyDetector(db, zscore_threshold=10.0)

        low_anomalies = low_threshold.check_today(_today())
        high_anomalies = high_threshold.check_today(_today())
...
class TestPatternAnomalies:
    ...
    def test_no_patterns_empty_db(self, db):
        ...
        patterns = detector.check_pattern_anomalies(_today())
        assert patterns == []

    def test_heavy_phone_usage_pattern(self, db):
        """Low battery + high screen events → heavy_phone_usage pattern."""
        today = _today()
        _insert_metric_day(db, "device.battery", today, 15.0)
        for i in range(55):
            e = Event(
                timestamp_utc=f"{today}T{8 + (i // 6):02d}:{(i * 10) % 60:02d}:00+00:00",
                timestamp_local=f"{today}T{3 + (i // 6):02d}:{(i * 10) % 60:02d}:00-05:00",
                timezone_offset="-0500",
        ...
        detector = AnomalyDetector(db, config=_PATTERN_CONFIG)
        patterns = detector.check_pattern_anomalies(today)
    ...
    def test_pattern_has_description(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "device.battery", today, 10.0)
        for i in range(60):
            e = Event(
                timestamp_utc=f"{today}T{8 + (i // 6):02d}:{(i * 10) % 60:02d}:00+00:00",
                timestamp_local=f"{today}T{3 + (i // 6):02d}:{(i * 10) % 60:02d}:00-05:00",
    ...
    def test_sleep_deprivation_high_stress(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "body.derived", today, 4.5, event_type="sleep_duration")
        _insert_metric_day(db, "mind.stress", today, 8.0)
    ...
    def test_sleep_deprivation_not_triggered_when_rested(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "body.derived", today, 8.0, event_type="sleep_duration")
        _insert_metric_day(db, "mind.stress", today, 8.0)
    ...
    def test_caffeine_late_poor_sleep(self, db):
        ...
        today = _today()
        e = Event(
            timestamp_utc=f"{today}T20:00:00+00:00",
            timestamp_local=f"{today}T15:00:00-05:00",
            timezone_offset="-0500",
        ...
        _insert_metric_day(db, "mind.sleep", today, 3.0)
    ...
    def test_low_mood_social_isolation(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "mind.mood", today, 2.0)
        _insert_metric_day(db, "social.derived", today, 5.0, event_type="density_score")
    ...
    def test_high_screen_low_movement(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "device.derived", today, 240.0, event_type="screen_time_minutes")
        _insert_metric_day(db, "body.steps", today, 1500.0)
    ...
    def test_cognitive_impairment_sleep_deprivation(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "cognition.derived", today, 3.5, event_type="cognitive_load_index")
        _insert_metric_day(db, "body.derived", today, 4.0, event_type="sleep_duration")
    ...
    def test_digital_restlessness_low_mood(self, db):
        ...
        today = _today()
        _insert_metric_day(db, "behavior.derived", today, 3.0, event_type="digital_restlessness")
        _insert_metric_day(db, "mind.mood", today, 2.0)
    ...
    def test_schumann_excursion_mood_swing(self, db):
        ...
        today = _today()
        e1 = Event(
            timestamp_utc=f"{today}T10:00:00+00:00",
            timestamp_local=f"{today}T05:00:00-05:00",
            timezone_offset="-0500",
        ...
        for val, hour in [(2.0, "08"), (9.0, "14"), (5.0, "20")]:
            e = Event(
                timestamp_utc=f"{today}T{hour}:00:00+00:00",
                timestamp_local=f"{today}T{hour}:00:00-05:00",
                timezone_offset="-0500",
    ...
    def test_fragmentation_caffeine_spike(self, db):
        ...
        today = _today()
        _insert_metric_day(
            db, "behavior.app_switch.derived", today, 75.0,
            event_type="fragmentation_index",
        )
        _insert_metric_day(db, "body.caffeine", today, 400.0)
